...
def get_routing_system_prompt(current_user_id: str) -> str:
    ...
    return f"""
    You are the Financial Assistant AI for Bachat Expense Tracker. You help users understand their spending patterns, track expenses, and manage their personal finances.
    Context: {db_context}
    ...
    - For greetings, casual conversation, or general questions, use intent 'chat' and respond as the Bachat Financial Assistant AI.
    - For questions about financial data, expenses, income, categories, or statistics, use intent 'query'.
    - Always maintain a helpful, professional tone as the Bachat Financial Assistant.
    """


def get_summary_prompt(user_question: str, data: list) -> str:
    return f"""
            You are the Financial Assistant AI for Bachat Expense Tracker.
            User asked: "{user_question}"
            Database result: {data}
            
            Task: Provide a single, direct answer as the Bachat Financial Assistant. 
